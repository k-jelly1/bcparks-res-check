...
-H 'x-app-version: bee882221532235305dc97be9d9203103a994042' \
...
alert_user() {
    ...
    echo "==================================="
    echo "FOUND PASSES FOR $FOUND_DATE!"
...
while true; do
    ...
    # Confirm that the server returned valid JSON.
    if ! echo "$RESPONSE" | jq empty >/dev/null 2>&1; then
        echo "Invalid API response received at $(date)"
        echo "$RESPONSE" | head -n 3
        sleep 30
        continue
    fi

    echo "$RESPONSE,$PARK_NAME,$(date)" | tee -a "$LOG"

    CAPACITY=$(echo "$RESPONSE" | jq -r --arg d "$DATE" \
        '.[$d].DAY.capacity // "Unavailable"')

    MAX=$(echo "$RESPONSE" | jq -r --arg d "$DATE" \
        '.[$d].DAY.max // 0')

    # Alert based on max > 0 rather than only capacity == Low.
    if [[ "$MAX" =~ ^[0-9]+$ ]] && [ "$MAX" -gt 0 ]; then
        if [ "$ALERTED_DATE1" = false ]; then
            alert_user "$DATE" "$CAPACITY" "$MAX"
            ALERTED_DATE1=true
        fi
    else
        # Allows another alert if availability disappears and later returns.
        ALERTED_DATE1=false
    fi

    if [ -n "$DATE2" ]; then
        CAPACITY2=$(echo "$RESPONSE" | jq -r --arg d "$DATE2" \
            '.[$d].DAY.capacity // "Unavailable"')
        ...
        if [[ "$MAX2" =~ ^[0-9]+$ ]] && [ "$MAX2" -gt 0 ]; then
            if [ "$ALERTED_DATE2" = false ]; then
                alert_user "$DATE2" "$CAPACITY2" "$MAX2"
                ALERTED_DATE2=true
            fi
        else
            ALERTED_DATE2=false
